...
def plot_routes(df):
    ...
    render = pdk.Deck(layers=[start_stations_layer, end_stations_layer, arc_layer],
                      initial_view_state=view_state,
                      tooltip=tooltip)


if __name__ == "__main__":
    df = load_data("202004-divvy-tripdata_clean.csv")  # Slides 8 and 9
    add_title_and_description()  # Slides 8 and 9
    show_number_of_trips(df)  # Slides 8 and 9
    show_all_stations_in_map(df)  # Slide 13
    filtered_df = add_station_name_filter(df)  # Slides 8 and 9
    column_1, column_2 = create_two_columns()  # Slide 11
    column_3, column_4 = create_two_columns()  # Slide 11
    plot_days_of_week(filtered_df, column_1)  # Slide 13
    plot_hours_of_day(filtered_df, column_2)  # Slide 13
    plot_member_type(filtered_df, column_3)  # Slide 13
    plot_average_duration_by_member_type(filtered_df, column_4)  # Slide 13
    plot_routes(filtered_df)  # Slide 13
